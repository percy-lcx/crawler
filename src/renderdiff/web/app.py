...
_index_checks: dict[str, _IndexState] = {}
...
def create_app() -> FastAPI:
    ...
    @app.post("/api/index", response_model=IndexCreated)
    async def start_index(req: IndexRequest) -> IndexCreated:
        urls = await resolve_urls(
            url=None,
            sitemap_url=req.sitemap,
            limit=req.limit,
        ...
        return IndexCreated(run_id=run_id, total_urls=len(urls))

    # -- Index SSE stream ---------------------------------------------------

    @app.get("/api/index/{run_id}/stream")
    async def stream_index(run_id: str) -> StreamingResponse:
        state = _index_checks.get(run_id)
        if not state:
            raise HTTPException(status_code=404, detail="Index check not found")

        return StreamingResponse(
            _sse_generator_index(state),
            media_type="text/event-stream",
            headers={
                "Cache-Control": "no-cache",
                "X-Accel-Buffering": "no",
    ...
    @app.get("/api/index/{run_id}")
    async def get_index(run_id: str) -> IndexStatusResponse:
        state = _index_checks.get(run_id)
        if not state:
            raise HTTPException(status_code=404, detail="Index check not found")
    ...
    @app.get("/api/index/{run_id}/screenshot/{filename}")
...
async def _sse_generator(state: _ScanState) -> AsyncGenerator[str, None]:
    """Yield SSE events from the scan's queue."""
    while True:
